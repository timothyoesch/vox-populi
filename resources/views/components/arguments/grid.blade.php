@php
    ...
    $arguments = collect(scandir(resource_path("content/" . app()->getLocale() . "/" . env("DECISION") . "/arguments")))->filter(function ($file) {
...
@endphp
<div class="koref-arguments mt-6 md:mt-12">
    @foreach ($arguments as $argument)
        <x-arguments.argument :title="$argument['name']">
            {!! $argument['content'] !!}
        </x-arguments.argument>
    @endforeach
</div>
...
<script>
    window.addEventListener("click", function (event) {
        ...
        if (!content) return;
        let icon = argument.querySelector(".koref-arguments__argument-title svg");
        if (!icon) return;
